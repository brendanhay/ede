...
module Text.EDE.Internal.Checker.MonadIO
    ( Check
    , runCheck
    , lift
    , throw
...
runCheck binds (Check c) = do
    ...
    c $ Env ref st (Map.fromList binds)
...
readTv (TM n ref) = Check $ \env -> do
    m <- readIORef (stack env)
    return . Right $ Map.lookup n m
-- readTcRef ref

writeTv :: String -> TMeta -> Tau -> Check ()
writeTv loc (TM n ref) ty = Check $ \env -> do
    print $ "writeTv" ++ show (loc, n, ty)
    writeIORef ref (Just ty)
    atomicModifyIORef' (stack env) ((, Right ()). Map.insert n ty)
...
quantify tvs ty = do
  ...
  where
    ...
    bind (tv, name) = writeTv "quantify" tv (TVar name)
...
zonkType (TMeta tv) = do    -- A mutable type variable
    ...
    case mb_ty of
        ...
        Just ty -> do
            ty' <- zonkType ty
            writeTv "zonkType" tv ty' -- "Short out" multiple hops
            return ty'
...
unifyUnboundVar tv1 ty2@(TMeta tv2) = do
    ...
    case mb_ty2 of
        ...
        Nothing  -> writeTv "unifyUnboundVar1" tv1 ty2

unifyUnboundVar tv1 ty2 = do
    tvs2 <- getMetaTyVars [ty2]
    if tv1 `elem` tvs2
        then occursCheckErr tv1 ty2
        else writeTv "unifyUnboundVar2" tv1 ty2
